...
// ADDED BY CHRIS COHEN FOR LAB 5 - GARBAGE COLLECTION
extern bool8 memblockkflag;
struct inusememblk {
  char    *memblockptr;       /* Ptr to in-use memory block */
  struct  inusememblk *mnext; /* Ptr to next list element */
};
void meminsert(struct inusememblk *, char *);
void memextract(struct inusememblk *, char *);
void print_freelist(pid32);
// END LAB 5 CODE
