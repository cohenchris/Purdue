...
void rtinit0() {
  ...
  dt0.costs[CURR_NODE][0] = 0;     // Cost from node 0 to node 0
  dt0.costs[CURR_NODE][1] = 1;     // Cost from node 0 to node 1
  dt0.costs[CURR_NODE][2] = 3;     // Cost from node 0 to node 2
  dt0.costs[CURR_NODE][3] = 7;     // Cost from node 0 to node 3

  dt0.costs[1][0] = INF;   // Cost from node 1 to node 0
  dt0.costs[1][1] = INF;   // Cost from node 1 to node 1
  dt0.costs[1][2] = INF;   // Cost from node 1 to node 2
  dt0.costs[1][3] = INF;   // Cost from node 1 to node 3

  dt0.costs[2][0] = INF;   // Cost from node 2 to node 0
  dt0.costs[2][1] = INF;   // Cost from node 2 to node 1
  dt0.costs[2][2] = INF;   // Cost from node 2 to node 2
  dt0.costs[2][3] = INF;   // Cost from node 2 to node 3

  dt0.costs[3][0] = INF;   // Cost from node 3 to node 0
  dt0.costs[3][1] = INF;   // Cost from node 3 to node 1
  dt0.costs[3][2] = INF;   // Cost from node 3 to node 2
  dt0.costs[3][3] = INF;   // Cost from node 3 to node 3

  // Initialize neighbor data structure - tells node if it's directly connected to another node
  neighbor0[0] = true;
  neighbor0[1] = true;
  neighbor0[2] = true;
  neighbor0[3] = true;
  ...
  // ( tolayer2() )
  for (int i = 0; i < 4; i++) {
    if ((i != CURR_NODE) && (neighbor0[i])) {
      printf("Node %d sending INITIAL cost from node %d to node %d --> %d\n", CURR_NODE, CURR_NODE, i, dt0.costs[CURR_NODE][i]);
      struct rtpkt neighbor_pkt = { 0 };
      neighbor_pkt.sourceid = CURR_NODE;
      neighbor_pkt.destid = i;
      neighbor_pkt.mincost[0] = dt0.costs[CURR_NODE][0];
      neighbor_pkt.mincost[1] = dt0.costs[CURR_NODE][1];
      neighbor_pkt.mincost[2] = dt0.costs[CURR_NODE][2];
      neighbor_pkt.mincost[3] = dt0.costs[CURR_NODE][3];
...
void rtupdate0(rtpkt *rcvdpkt) {
  printf("rtupdate0 received a packet that was sent from node %d at %f\n", rcvdpkt->sourceid, clocktime);
  bool updated = false;
  bool new_min = false;
  int min = INF;
  ...
  for (int i = 0; i < 4; i++) {
    //if (rcvdpkt->mincost[i] < dt0.costs[rcvdpkt->sourceid][i]) {
  ...
  for (int y = 0; y < 4; y++) {
    min = dt0.costs[CURR_NODE][y];
    new_min = false;
    // Find the min over all neighbors 'n' of CURR_NODE
    for (int n = 0; n < 4; n++) {
      if ((dt0.costs[CURR_NODE][n] + dt0.costs[n][y]) < min) {
        min = dt0.costs[CURR_NODE][n] + dt0.costs[n][y];
        updated = true;
        new_min = true;
      }
    }
  ...
  for (int i = 0; i < 4; i++) {
    if ((i != CURR_NODE) && (neighbor0[i]) && (updated)) {
      struct rtpkt neighbor_pkt = { 0 };
      neighbor_pkt.sourceid = CURR_NODE;
      neighbor_pkt.destid = i;
      neighbor_pkt.mincost[0] = dt0.costs[CURR_NODE][0];
